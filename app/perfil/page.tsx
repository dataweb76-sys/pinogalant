    ...
    <main style={{ maxWidth: 760, margin: "0 auto", padding: 24, display: "grid", gap: 14 }}>
      ...
      {searchParams?.ok ? (
        <div className="card" style={{ border: "1px solid #d1fae5", background: "#ecfdf5", padding: 12 }}>
          ✅ {searchParams.ok === "pass" ? "Contraseña actualizada" : "Guardado"}
        </div>
      ) : null}
      ...
      <form action={saveProfileAction} className="card" style={{ display: "grid", gap: 12, padding: 16 }}>
        <div>
      ...
      <form action={changePasswordAction} className="card" style={{ display: "grid", gap: 12, padding: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <input className="input" type="password" name="password" minLength={8} placeholder="mínimo 8" required />
          </div>
          <div>
            <label className="small">Repetir contraseña</label>
            <input className="input" type="password" name="password2" minLength={8} placeholder="repetir" required />
          </div>
        </div>
        ...
        <div className="small" style={{ opacity: 0.7 }}>
          Tip: si querés también “Olvidé mi contraseña”, después agregamos el flujo de reset por email.
        </div>
      </form>
    </main>
  );
