...
export async function changePasswordAction(formData: FormData) {
  ...
  if (!password || password.length < 8) redirect(`/perfil?error=${encodeURIComponent("La contraseña debe tener mínimo 8 caracteres")}`);
  if (password !== password2) redirect(`/perfil?error=${encodeURIComponent("Las contraseñas no coinciden")}`);
